...
export function NotesWorkspace({ personal = false }: { personal?: boolean }) {
  ...
  function scheduleScratch(value: string) {
    ...
    timer.current = setTimeout(async () => {
      setSaving(true)
      const response = await fetch(scratchId.current ? `/api/notes/${scratchId.current}` : '/api/notes', {
        method: scratchId.current ? 'PATCH' : 'POST',
        headers: { 'content-type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify(scratchId.current ? { content: value } : {
          content: value,
          kind: 'scratch',
      ...
      const data = await response.json()
      if (response.ok) {
        scratchId.current = data.note.id
        await load()
      }
      setSaving(false)
    }, 600)
      ...
      <section className="workspace-stack">
        ...
        {scratchId.current && (
          <div className="inline-actions">
            <button className="button button-ghost button-sm" onClick={() => convert(scratchId.current!, 'note')}>메모로 전환</button>
            <button className="button button-ghost button-sm" onClick={() => convert(scratchId.current!, 'task')}>할 일로 전환</button>
          </div>
        )}
